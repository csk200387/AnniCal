        ...
        <section class="overflow-hidden border hairline bg-paper-50" aria-label="월간 달력">
          ...
          <div class="grid grid-cols-7">
            <button
              ...
              type="button"
              class="group relative flex min-h-[72px] flex-col items-stretch gap-2 border-b border-r border-rule p-2 text-left transition-colors duration-200 ease-out sm:min-h-[106px] sm:p-2.5"
              :class="{
                'hover:bg-paper-200/40': !selectedDate.isSame(cell.date, 'day'),
                'bg-paper-100/50 text-ink-300': !cell.isCurrentMonth,
                'bg-ink-900 text-paper-50 hover:bg-ink-700': selectedDate.isSame(cell.date, 'day'),
              }"
            ...
            >
              ...
              <div v-if="cell.anniversaries.length" class="flex flex-wrap items-center gap-1 sm:hidden">
                ...
                <span v-if="cell.anniversaries.length > 4" class="text-[9px] transition-colors duration-200 ease-out" :class="selectedDate.isSame(cell.date, 'day') ? 'text-paper-300' : 'text-ink-400'">+{{ cell.anniversaries.length - 4 }}</span>
              </div>
              <div v-if="cell.anniversaries.length" class="hidden min-w-0 flex-col gap-1 overflow-hidden sm:flex">
                ...
                <span v-if="cell.anniversaries.length > 2" class="text-[9px] transition-colors duration-200 ease-out" :class="selectedDate.isSame(cell.date, 'day') ? 'text-paper-300' : 'text-ink-400'">+{{ cell.anniversaries.length - 2 }} more</span>
              </div>
            </button>
          </div>
        </section>
